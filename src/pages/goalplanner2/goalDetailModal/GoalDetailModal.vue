      ...
      <div v-else key="view-detail" class="modal-body-full">
        ...
        <div class="ms-workspace-header">
          ...
          <div class="header-right">
            ...
            <button 
              @click="removeMilestone(activeMilestone?.id)" 
              class="btn-text-danger ml-4"
            >
              마일스톤 삭제
            </button>
          </div>
...
const openMilestoneDetail = (ms: ScheduleItem) => {
  ...
  // 캘린더 초기화: v-model로 연결된 selectedMsDate만 바꿔주면 Calendar 내부에서 월(Month) 이동을 자동으로 처리
...
const onSummaryChange = (e: Event) => {
  ...
  if (target) {
    handleMilestoneUpdate({ summary: target.value });
  }
}
...
/* 캘린더 패널 래퍼 (기존 시각적 스타일 제거, Calendar가 대신 처리) */
.cal-panel { 
  flex: 1 1 450px; 
  max-width: 450px; 
  min-width: 300px; 
